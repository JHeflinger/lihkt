...
elements = ["theme", "full_name", "email", "phone_number", "address", "address_2", "education"]
fileElements = []
appRunning = True
saved = True
fileName = ""
    ...
    if theme in fileList:
        ...
        for t in themeContent:
            ...
            if content != "":
                ...
                pdf.cell(w = int(define[2]), h = int(define[3]), txt = define[4] + content, border = define[5], ln = int(define[6]), align = define[7], fill = define[8], link = define[9][0:len(define[9]) - 1])
