...
class ComplexElement(QWidget):
    def __init__(self, header, elementContent, parent):
        ...
        for content in self.contents:
            ...
            #self.layouts.append(widgetRow)
            widget.textChanged.connect(self.parent.unSave)
            #widgetRow.addWidget(widget)
            #delBtn = QPushButton("delete")
            #delBtn.clicked.connect(lambda:self.delete(widgetRow))
            #widgetRow.addWidget(delBtn)
            widgetRow = self.WidgetRow(widget, self, index)
            self.activeWidgets.append(index)
            self.layout.addWidget(widgetRow)
            index += 1
    ...
    def addContent(self):
        ...
        self.widgets.append(widget)
        widget.textChanged.connect(self.parent.unSave)
        widgetRow = self.WidgetRow(widget, self, self.activeWidgets[len(self.activeWidgets) - 1] + 1)
        self.activeWidgets.append(self.activeWidgets[len(self.activeWidgets) - 1] + 1)
        self.layout.addWidget(widgetRow)
        
    def getContentText(self):
        str = ""
        for id in self.activeWidgets:
            str += (self.widgets[id].text() + "|")
    ...
    class WidgetRow(QWidget):
        ...
        def delete(self):
            ...
            self.parent.activeWidgets.remove(self.index)
